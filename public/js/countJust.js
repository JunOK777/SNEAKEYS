$(document).ready(function () {
  // want
  $('#count02').on('click', function() {
    let image_id = $('#image_id').val()

    let url01      = `/image/${image_id}/just/null`
    let url02      = `/image/${image_id}/just/just`
    let url03      = `/image/${image_id}/just/other`

    let _token   = $('meta[name="csrf-token"]').attr('content')

    // want
     if ( $('#count02').hasClass("done")){
      ...
      $.post(url03, { _token: _token},
        function (match) {
          if(match.success === true){
            ...
            $('#count02').removeClass('circle').addClass('done')
